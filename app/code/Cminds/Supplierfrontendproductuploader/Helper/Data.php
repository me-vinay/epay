...
class Data extends AbstractHelper
{
     ...
     */
    public function getSupplierDisableAttributeValues($customer)
    {
        $disableAttributesJson = $customer
            ->getCustomAttribute('supplier_disable_attribute_values');

        if (!$disableAttributesJson) {
            return [];
        }

        try {
            return $this->jsonSerializer->unserialize($disableAttributesJson->getValue() ?? '');
        } catch (\Exception $e) {
            return [];
        }
    }

    public function disableProductsByAttribute($customer)
    {
        $disableValues = $this->getSupplierDisableAttributeValues($customer);
        if (!$disableValues) {
            return;
        }

        $productCollection = $this->productCollectionFactory->create()
            ->addAttributeToFilter($this->config->getDisableAttributeCode(), ['in' => $disableValues])
            ->addAttributeToFilter('creator_id', $customer->getId());

        foreach ($productCollection as $product) {
            $product->setSupplierActivedProduct(0);
            $product->setVisibility(Visibility::VISIBILITY_NOT_VISIBLE);
            $product->getResource()->saveAttribute($product, 'supplier_actived_product');

            $product->setFrontendproductProductStatus(CmindsProduct::STATUS_NONACTIVE);
